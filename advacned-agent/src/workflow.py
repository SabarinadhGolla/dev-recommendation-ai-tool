...
class Workflow:
    ...
    def _extract_tools_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        for result in results_list:
            ...
            if url:
                scarped = self.firecrawl.scrape_company_page(url)
                if scarped and hasattr(scarped, 'markdown') and scarped.markdown:
                    all_content += scarped.markdown[:1500] + "\n\n"
        ...
        try:
            ...
            tool_names = [
                name.strip() for name in response.content.split("\n") if name.strip()
            ]
    ...
    def _research_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        if not extracted_tools:
            ...
            if search_results and isinstance(search_results, dict):
                ...
                tool_names = [
                    ...
                    for result in results_list if isinstance(result, dict)
                ]
        else:
        ...
        for tool_name in tool_names:
            ...
            if tool_search_results:
                results_list = tool_search_results.get("data", []) if isinstance(tool_search_results, dict) else []
                if not results_list:
                    continue
    ...
    def _analyze_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        messages = [
            ...
            HumanMessage(content=self.prompts.recommendations_user(state.query, str(company_data)))
        ]
